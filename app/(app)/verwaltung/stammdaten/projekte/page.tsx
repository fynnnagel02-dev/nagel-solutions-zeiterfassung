            ...
            <tbody>
              {projects.map((project) => (
                <tr key={project.id} className="border-t border-[color:var(--color-border-soft)] align-top">
                  ...
                  <td className="px-5 py-4 text-[color:var(--color-text-soft)]">{project.description || "Keine Beschreibung hinterlegt."}</td>
                  <td className="px-5 py-4">
                    <StatusChip label={project.is_active ? "Aktiv" : "Inaktiv"} tone={project.is_active ? "success" : "neutral"} />
                  </td>
                  <td className="px-5 py-4 text-[color:var(--color-text-soft)]">{formatDateTime(project.updated_at)}</td>
                  <td className="px-5 py-4"><ProjectRowActions project={project} /></td>
                </tr>
              ))}
            </tbody>
          </table>
