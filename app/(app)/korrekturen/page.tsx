import { CorrectionRequestForm, WithdrawCorrectionButton } from "@/src/components/employee/CorrectionRequestForm";
...
import { getMyCorrectionOverview } from "@/src/lib/db/queries/frontoffice";
...
export default async function CorrectionsPage() {
  ...
  const overview = await getMyCorrectionOverview();
        ...
        <div className="space-y-4">
          {overview.eligibleEntries.length === 0 ? (
            ...
            overview.eligibleEntries.map((entry) => (
              <Panel key={entry.id} className="space-y-3 border-[color:var(--color-border-soft)] bg-[color:var(--color-panel-soft)]">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <CorrectionRequestForm timeEntryId={entry.id} />
                </div>
              </Panel>
            ))
          )}
