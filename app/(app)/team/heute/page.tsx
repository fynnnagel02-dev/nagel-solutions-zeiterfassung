              ...
              <Panel key={item.employee.id} className="space-y-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="space-y-2">
                    ...
                    <p className="text-sm text-[color:var(--color-text-soft)]">
                      {item.activeAbsence
                        ? `Abwesenheit: ${item.activeAbsence.leave_type === "sick" ? "Krank" : "Geplant"}`
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  <div className="rounded-[1.5rem] border border-[color:var(--color-border-soft)] bg-[color:var(--color-panel-soft)] p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-[color:var(--color-text-muted)]">Arbeitszeit</p>
                    <p className="mt-2 text-lg font-semibold tabular-nums text-[color:var(--color-text)]">
                      {item.timeEntry ? formatMinutes(item.timeEntry.computedWorkedMinutes) : "--"}
                    </p>
                  </div>
                  <div className="rounded-[1.5rem] border border-[color:var(--color-border-soft)] bg-[color:var(--color-panel-soft)] p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-[color:var(--color-text-muted)]">Pause</p>
                    <p className="mt-2 text-lg font-semibold tabular-nums text-[color:var(--color-text)]">
                      {item.timeEntry ? formatMinutes(item.timeEntry.computedBreakMinutes) : "--"}
                    </p>
                  </div>
                  <div className="rounded-[1.5rem] border border-[color:var(--color-border-soft)] bg-[color:var(--color-panel-soft)] p-4">
                    <p className="text-xs uppercase tracking-[0.18em] text-[color:var(--color-text-muted)]">Hinweis</p>
                    <p className="mt-2 text-sm text-[color:var(--color-text-soft)]">
                      {item.timeEntry ? "Eintrag vorhanden" : item.activeAbsence ? "Abwesenheit aktiv" : "Noch keine Buchung"}
                    </p>
                  </div>
                </div>
              </Panel>
