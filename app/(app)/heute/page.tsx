...
export default async function TodayPage() {
  ...
  const [company, entry, week] = await Promise.all([
    getCompanyContext(),
    getMyTodayTimeEntry(),
    getMyWeekOverview(),
  ]);
      ...
      <div className="grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
        <Panel className="space-y-5">
          <div className="flex items-start justify-between gap-4">
            ...
            <Link href="/woche" className="text-sm font-semibold text-[color:var(--color-sidebar)]">
              Wochenansicht
            </Link>
          </div>
        ...
        <Panel className="space-y-5">
          ...
          <div className="space-y-3">
            ...
            <div className="rounded-[1.5rem] border border-[color:var(--color-border-soft)] bg-[color:var(--color-panel-soft)] p-4 text-sm leading-6 text-[color:var(--color-text-soft)]">
              Wenn heute keine Buchung vorliegt, erfassen Sie Nachträge in der Wochenansicht. Urlaub und Korrekturen
              bleiben eigene Prozesse, greifen aber auf denselben serverbestätigten Arbeitszeitkontext zurück.
            </div>
          </div>
        </Panel>
      </div>
